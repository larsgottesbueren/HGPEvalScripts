...
def combined_pp_rs(df, fig, baseline, colors=None, algos=None, instances=None, time_limit=7200):
	if algos == None:
	...
	outer_grid = matplotlib.gridspec.GridSpec(nrows=1, ncols=2, wspace=0.02, hspace=0.0, width_ratios=[0.5,0.5])

	## separate plots
	ratios_df = performance_profiles.performance_profiles(algos, instances, df, objective="km1")
	handles, labels = performance_profiles.plot(ratios_df, fig=fig, external_subplot=outer_grid[0], colors=colors, width_scale=1.0)
	...
	fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, -0.05), frameon=False, ncol=4)
